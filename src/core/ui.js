(function init(root, factory) {
  ...
  function createDialog(title='') {
    ...
    const closeButton=e('button','bt-icon-button'); closeButton.type='button'; closeButton.title='关闭'; closeButton.textContent='×';
  ...
  function navLink(href,label,iconName,callbacks,cls='bt-nav-item') { const a=e('a',cls); a.href=href; a.append(icon(iconName),e('span','bt-nav-label',label)); return a; }
  ...
  function createTopbar(data={},callbacks={}) {
    ...
    const suggestions=e('div','bt-search-suggestions');suggestions.hidden=true;suggestions.setAttribute('role','listbox');
    let suggestionTimer=null,suggestionSeq=0,suggestionItems=[],activeIndex=-1;
    const hideSuggestions=()=>{suggestions.hidden=true;activeIndex=-1;};
    ...
    const setActive=(next)=>{if(!suggestionItems.length)return;activeIndex=(next+suggestionItems.length)%suggestionItems.length;Array.from(suggestions.children).forEach((node,index)=>node.classList.toggle('is-active',index===activeIndex));};
    const renderSuggestions=(items)=>{suggestionItems=Array.isArray(items)?items.slice(0,10):[];suggestions.replaceChildren();activeIndex=-1;if(!suggestionItems.length){hideSuggestions();return;}for(const value of suggestionItems){const row=e('button','bt-search-suggestion');row.type='button';row.setAttribute('role','option');row.append(icon('search'),e('span','',value));row.addEventListener('mousedown',ev=>ev.preventDefault());row.addEventListener('click',()=>{input.value=value;hideSuggestions();searchFor(value);});suggestions.append(row);}suggestions.hidden=false;};
    ...
    input.addEventListener('focus',()=>{if(suggestionItems.length)suggestions.hidden=false;});input.addEventListener('blur',()=>setTimeout(hideSuggestions,120));
  ...
  function sideSection(title, items, callbacks) { const section=e('section','bt-side-section'); section.dataset.btGroup=title||'primary'; if(title)section.append(e('h2','bt-side-title',title)); for(const item of items){if(item.action&&callbacks&&typeof callbacks[item.action]==='function'){const b=e('button','bt-nav-item bt-nav-button');b.type='button';b.append(icon(item.icon),e('span','bt-nav-label',item.label));b.addEventListener('click',callbacks[item.action]);section.append(b);}else section.append(navLink(item.href,item.label,item.icon,callbacks));} return section; }
  ...
  function createSidebar(data={},callbacks={}) {
    ...
    scroller.append(sideSection('',[
      {label:'首页',icon:'home',href:'https://www.bilibili.com/'},{label:'动态',icon:'dynamic',href:'https://t.bilibili.com/'},{label:'热门',icon:'hot',href:'https://www.bilibili.com/v/popular/all/'},
    ],callbacks),sideSection('你的内容',[
      {label:'历史记录',icon:'history',href:'https://www.bilibili.com/history'},{label:'稍后再看',icon:'later',href:'https://www.bilibili.com/list/watchlater'},{label:'收藏夹',icon:'library',href:data.user&&data.user.mid?`https://space.bilibili.com/${data.user.mid}/favlist`:'https://www.bilibili.com/'},
    ],callbacks));
    ...
    scroller.append(sideSection('探索',[
      {label:'直播',icon:'live',href:'https://live.bilibili.com/'},{label:'番剧影视',icon:'anime',href:'https://www.bilibili.com/anime/'},{label:'知识',icon:'knowledge',href:'https://www.bilibili.com/v/knowledge/'},{label:'科技',icon:'tech',href:'https://www.bilibili.com/v/tech/'},{label:'游戏',icon:'game',href:'https://www.bilibili.com/v/game/'},
    ],callbacks),sideSection('更多',[
      {label:'消息',icon:'message',href:'https://message.bilibili.com/'},{label:'大会员',icon:'premium',href:'https://account.bilibili.com/big'},{label:'设置',icon:'settings',action:'openSettings'},
    ],callbacks));
  ...
  function progressiveImage(src,box,priority=false){
    ...
    img.alt='';img.loading=priority?'eager':'lazy';img.fetchPriority=priority?'high':'auto';img.decoding='async';img.dataset.btFullSrc=full;
  ...
  function imageBox(item){const box=e('div','bt-thumb');if(item.thumbnail){const img=progressiveImage(item.thumbnail,box,Boolean(item.imagePriority));box.append(img,e('span','bt-thumb-progress-shimmer'));}else box.append(e('div','bt-thumb-placeholder','BiliTube'));if(item.duration)box.append(e('span','bt-duration',''+item.duration));return box;}
  ...
  function watchLaterQuick(item,callbacks={}){
    ...
    const stop=(ev)=>{ev.preventDefault();ev.stopPropagation();};button.addEventListener('pointerdown',stop);button.addEventListener('click',async(ev)=>{stop(ev);if(button.classList.contains('is-loading'))return;button.classList.add('is-loading');const next=await callbacks.toggleWatchLater(item,button);button.classList.remove('is-loading');if(typeof next==='boolean'){button.classList.toggle('is-active',next);button.setAttribute('aria-pressed',String(next));button.title=next?'从稍后再看移除':'稍后再看';button.setAttribute('aria-label',button.title);}});return button;
  }
  function createCard(item,callbacks={}){const card=e('article','bt-video-card');const media=e('div','bt-card-media');const thumbLink=e('a','bt-thumb-link');bindLink(thumbLink,item.href,callbacks);const thumb=imageBox(item);thumbLink.append(thumb);media.append(thumbLink);const quick=watchLaterQuick(item,callbacks);if(quick)media.append(quick);card.append(media);const row=e('div','bt-card-row');const avLink=e('a','bt-avatar-link');bindLink(avLink,item.authorHref||item.href,callbacks);avLink.append(avatar(item));const info=e('div','bt-card-info');const title=e('a','bt-card-title',item.title||'未命名视频');bindLink(title,item.href,callbacks);info.append(title);if(item.author){const author=e('a','bt-card-author',item.author);bindLink(author,item.authorHref||item.href,callbacks);info.append(author);}if(item.meta)info.append(e('div','bt-card-meta',item.meta));row.append(avLink,info);card.append(row);if(callbacks.bindPreview)callbacks.bindPreview(card,item,thumb);return card;}
  function heading(title,sub){const h=e('div','bt-page-heading');h.append(e('h1','',title));if(sub)h.append(e('p','',sub));return h;}
  function renderGrid(outlet,items,callbacks){if(!items||!items.length){outlet.append(e('div','bt-empty','暂时没有读取到内容'));return;}const grid=e('section','bt-video-grid');for(const [index,item] of items.entries())grid.append(createCard(index<4?{...item,imagePriority:true}:item,callbacks));outlet.append(grid);}
  ...
  function renderHomeShelf(outlet,section,callbacks){
    ...
    const row=e('div','bt-home-shelf-row');for(const [index,item] of section.items.entries())row.append(createShelfCard(index<4?{...item,imagePriority:true}:item,callbacks));
  ...
  function renderHomeVideoBlock(outlet,items,callbacks,label=''){
    if(!items||!items.length)return;if(label)outlet.append(e('h2','bt-home-block-title',label));const grid=e('section','bt-video-grid bt-home-grid');for(const [index,item] of items.entries())grid.append(createCard(index<4?{...item,imagePriority:true}:item,callbacks));outlet.append(grid);
  }
  ...
  function renderHome(outlet,data,callbacks){
    ...
    const videos=data.videos||[];const shelves=Array.isArray(data.shelves)?data.shelves:[];let cursor=0;
    renderHomeVideoBlock(outlet,videos.slice(cursor,cursor+8),callbacks,'为你推荐');cursor+=8;
    for(const shelf of shelves){renderHomeShelf(outlet,shelf,callbacks);renderHomeVideoBlock(outlet,videos.slice(cursor,cursor+8),callbacks,'继续为你推荐');cursor+=8;}
    renderHomeVideoBlock(outlet,videos.slice(cursor),callbacks,cursor?'继续为你推荐':'为你推荐');
  ...
  function renderDynamic(outlet,data,callbacks){
    const items=data.items||[];const mode=String(data.mode||'all');const videos=items.filter(item=>item.video).map(item=>item.video);const others=items.filter(item=>!item.video);
    ...
    if(mode!=='updates'&&videos.length){const grid=e('section','bt-video-grid bt-dynamic-video-grid');for(const video of videos)grid.append(createCard(video,callbacks));outlet.append(grid);}else if(mode==='videos')outlet.append(e('div','bt-empty',data.error||'暂时没有投稿视频'));
    ...
    if(!videos.length&&!others.length)outlet.append(e('div','bt-empty',data.loading?'正在读取动态…':data.error||'暂时没有读取到动态'));
